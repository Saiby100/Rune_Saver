    ...
    def close(self, caller):
        ...
        for img_btn in self.children:
            if img_btn.name == caller.name:
                ...
                anim.start(img_btn)
        self.show_title()
        if caller.name in config.runes.keys():
...
class IconDrawerManager(MDRelativeLayout):
    '''
        This manages communication between drawer objects.
    '''
    ...
    def add_corresponding_drawers(self, main_rune):
        ...
        if self.is_primary:
            for key in config.runes[main_rune]:
                drawer = IconDrawer(row=config.runes[main_rune][key],
                                    pos_hint={'top': 1},
                                    opacity=0)
                self.add_widget(drawer)
        else:
            for i in range(2):
                drawer = IconDrawer(row=config.secondary_runes[main_rune],
                                    pos_hint={'top': 1},
                                    opacity=0)
                self.add_widget(drawer)
